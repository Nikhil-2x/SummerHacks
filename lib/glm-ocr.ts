...
const ZAI_MODEL_NAME = "glm-4.6v-flash";
...
export async function runGlmOcr({
  base64,
  mimeType,
}: GlmOcrParams): Promise<GlmOcrResult> {
  if (!ZAI_API_KEY) {
    throw new Error(
      "GLM-OCR is not configured. Please set ZAI_API_KEY on the server.",
    );
  }
  ...
  if (!markdown) {
    ...
    throw new Error(
      "OCR did not return any text. Please ensure the report is clear and readable.",
    );
  }
